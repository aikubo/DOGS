...
#include "Water97NoError.h"
#include "MathUtils.h"
#include "libmesh/utility.h"

registerMooseObject("dikesApp", Water97NoError);

InputParameters
Water97NoError::validParams()
{
  InputParameters params = SinglePhaseFluidProperties::validParams();
  params.addClassDescription("Fluid properties for water and steam (H2O) using IAPWS-IF97");
  params.addParam<bool>("error_on_out_of_bounds", false, "If true, timesteps will be cut after out of bounds, if false it just gives a warning");
  
  return params;
}

Water97NoError::Water97NoError(const InputParameters & parameters)
  : SinglePhaseFluidProperties(parameters),
    ...
    _p_triple(611.657),
    _T_triple(273.16),
    _error(getParam<bool>("error_on_out_of_bounds"))
{
}

Water97NoError::~Water97NoError() {}

std::string
Water97NoError::fluidName() const
{
  return "water";
}

Real
Water97NoError::molarMass() const
{
  return _Mh2o;
}

Real
Water97NoError::criticalPressure() const
{
  return _p_critical;
}

Real
Water97NoError::criticalTemperature() const
{
  return _T_critical;
}

Real
Water97NoError::criticalDensity() const
{
  return _rho_critical;
}

Real
Water97NoError::triplePointPressure() const
{
  return _p_triple;
}

Real
Water97NoError::triplePointTemperature() const
{
  return _T_triple;
}

Real
Water97NoError::rho_from_p_T(Real pressure, Real temperature) const
{
  Real density, pi, tau;
...
Real
Water97NoError::e_from_p_T(Real pressure, Real temperature) const
{
...
Real
Water97NoError::c_from_p_T(Real pressure, Real temperature) const
{
...
Real
Water97NoError::cp_from_p_T(Real pressure, Real temperature) const
{
...
Real
Water97NoError::cv_from_p_T(Real pressure, Real temperature) const
{
  Real specific_heat, pi, tau, delta;
...
Real
Water97NoError::mu_from_p_T(Real pressure, Real temperature) const
{
...
Real
Water97NoError::mu_from_rho_T(Real density, Real temperature) const
{
...
void
Water97NoError::mu_from_rho_T(Real density,
                                      Real temperature,
                                      Real ddensity_dT,
                                      Real & mu,
                                      Real & dmu_drho,
...
void
Water97NoError::rho_mu_from_p_T(Real pressure,
                                        Real temperature,
                                        Real & rho,
                                        Real & mu) const
{
...
Real
Water97NoError::k_from_p_T(Real pressure, Real temperature) const
{
  Real rho = this->rho_from_p_T(pressure, temperature);
...
void
Water97NoError::k_from_p_T(Real, Real, Real &, Real &, Real &) const
{
  mooseError(name(), ": k_from_p_T() is not implemented.");
}

Real
Water97NoError::k_from_rho_T(Real density, Real temperature) const
{
...
Real
Water97NoError::s_from_p_T(Real pressure, Real temperature) const
{
...
void
Water97NoError::s_from_p_T(Real p, Real T, Real & s, Real & ds_dp, Real & ds_dT) const
{
  SinglePhaseFluidProperties::s_from_p_T(p, T, s, ds_dp, ds_dT);
}

Real
Water97NoError::h_from_p_T(Real pressure, Real temperature) const
{
  ...
  switch (region)
  {
    ...
    default:
      mooseError("Water97NoError::inRegion has given an incorrect region");
  }
  return enthalpy;
}

void
Water97NoError::h_from_p_T(
...
Real
Water97NoError::vaporPressure(Real temperature) const
{
  ...
  if (temperature < 273.15 || temperature > _T_critical) {
    if (_error)
      ...
      // mooseWarning(name(),
      //              ": vaporPressure(): Temperature is outside range 273.15 K <= T <= 647.096 K",
      //              "Continuing anyway but results may not be valid in this range.");
    }
  }
...
void
Water97NoError::vaporPressure(Real temperature, Real & psat, Real & dpsat_dT) const
{
  // Check whether the input temperature is within the region of validity of this equation.
  // Valid for 273.15 K <= t <= 647.096 K
  if (temperature < 273.15 || temperature > _T_critical) {
...
FPDualReal
Water97NoError::vaporTemperature_ad(const FPDualReal & pressure) const
{
...
Real
Water97NoError::vaporTemperature(Real pressure) const
{
...
void
Water97NoError::vaporTemperature(Real pressure, Real & Tsat, Real & dTsat_dp) const
{
...
Real
Water97NoError::b23p(Real temperature) const
{
  // Check whether the input temperature is within the region of validity of this equation.
...
Real
Water97NoError::b23T(Real pressure) const
{
  // Check whether the input pressure is within the region of validity of this equation.
...
unsigned int
Water97NoError::inRegion(Real pressure, Real temperature) const
{
  ...
  if (temperature >= 273.15 && temperature <= 1073.15)
  {
    if (pressure < vaporPressure(273.15) || pressure > 100.0e6) {
      ...
      {
        // mooseWarning("Pressure ", pressure, " is out of range in ", name(), ": inRegion()",
        //             "Continuing anyway but results may not be valid in this range.");
        region = 2;
  ...
  else if (temperature > 1073.15 && temperature <= 2273.15)
  {
    ...
    { 
      // mooseWarning("Temperature ", temperature, " is out of range in ", name(), ": inRegion()",
      //             "Continuing anyway but results may not be valid in this range.");
      region = 2;
...
Real
Water97NoError::gamma1(Real pi, Real tau) const
{
  Real sum = 0.0;
...
Real
Water97NoError::dgamma1_dpi(Real pi, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 0; i < _n1.size(); ++i)
...
Real
Water97NoError::d2gamma1_dpi2(Real pi, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 0; i < _n1.size(); ++i)
...
Real
Water97NoError::dgamma1_dtau(Real pi, Real tau) const
{
  Real g = 0.0;
  for (std::size_t i = 0; i < _n1.size(); ++i)
...
Real
Water97NoError::d2gamma1_dtau2(Real pi, Real tau) const
{
  Real dg = 0.0;
  for (std::size_t i = 0; i < _n1.size(); ++i)
...
Real
Water97NoError::d2gamma1_dpitau(Real pi, Real tau) const
{
  Real dg = 0.0;
  for (std::size_t i = 0; i < _n1.size(); ++i)
...
Real
Water97NoError::gamma2(Real pi, Real tau) const
{
...
Real
Water97NoError::dgamma2_dpi(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 1.0 / pi;
...
Real
Water97NoError::d2gamma2_dpi2(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = -1.0 / pi / pi;
...
Real
Water97NoError::dgamma2_dtau(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 0.0;
  for (std::size_t i = 0; i < _n02.size(); ++i)
...
Real
Water97NoError::d2gamma2_dtau2(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 0.0;
  for (std::size_t i = 0; i < _n02.size(); ++i)
...
Real
Water97NoError::d2gamma2_dpitau(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 0.0;
...
Real
Water97NoError::phi3(Real delta, Real tau) const
{
  Real sum = 0.0;
...
Real
Water97NoError::dphi3_ddelta(Real delta, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 1; i < _n3.size(); ++i)
...
Real
Water97NoError::d2phi3_ddelta2(Real delta, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 1; i < _n3.size(); ++i)
...
Real
Water97NoError::dphi3_dtau(Real delta, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 1; i < _n3.size(); ++i)
...
Real
Water97NoError::d2phi3_dtau2(Real delta, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 1; i < _n3.size(); ++i)
...
Real
Water97NoError::d2phi3_ddeltatau(Real delta, Real tau) const
{
  Real sum = 0.0;
  for (std::size_t i = 1; i < _n3.size(); ++i)
...
Real
Water97NoError::gamma5(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real sum0 = 0.0;
...
Real
Water97NoError::dgamma5_dpi(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 1.0 / pi;
...
Real
Water97NoError::d2gamma5_dpi2(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = -1.0 / pi / pi;
...
Real
Water97NoError::dgamma5_dtau(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 0.0;
  for (std::size_t i = 0; i < _n05.size(); ++i)
...
Real
Water97NoError::d2gamma5_dtau2(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 0.0;
  for (std::size_t i = 0; i < _n05.size(); ++i)
...
Real
Water97NoError::d2gamma5_dpitau(Real pi, Real tau) const
{
  // Ideal gas part of the Gibbs free energy
  Real dg0 = 0.0;
...
unsigned int
Water97NoError::subregion3(Real pressure, Real temperature) const
{
...
Real
Water97NoError::tempXY(Real pressure, subregionEnum xy) const
{
...
Real
Water97NoError::subregionVolume(
...
Real
Water97NoError::densityRegion3(Real pressure, Real temperature) const
{
...
unsigned int
Water97NoError::inRegionPH(Real pressure, Real enthalpy) const
{
  unsigned int region;
  ...
  if (pressure >= p273 && pressure <= p623)
  {
    if (enthalpy >= h_from_p_T(pressure, 273.15) &&
      ...
      if (_error)
        ...
        region = 2;
        // mooseWarning("Enthalpy ", enthalpy, " is out of range in ", name(), ": inRegionPH()",
        //               " setting region to 5, but this may cause incorrect results");
      }
    }
  }
  ...
  else if (pressure > 50.0e6 && pressure <= 100.0e6)
  {
    ...
    if (_error)
      ...
      region = 2;
      // mooseWarning("Pressure ", pressure, " is out of range in ", name(), ": inRegionPH()",
      //               " setting region to 5, but this may cause incorrect results");
    }
  }
...
unsigned int
Water97NoError::subregion2ph(Real pressure, Real enthalpy) const
{
...
unsigned int
Water97NoError::subregion3ph(Real pressure, Real enthalpy) const
{
  unsigned int subregion;
...
Real
Water97NoError::b2bc(Real pressure) const
{
  // Check whether the input pressure is within the region of validity of this equation.
...
Real
Water97NoError::T_from_p_h(Real pressure, Real enthalpy) const
{
  const FPDualReal p = pressure;
...
FPDualReal
Water97NoError::T_from_p_h_ad(const FPDualReal & pressure,
...
FPDualReal
Water97NoError::temperature_from_ph1(const FPDualReal & pressure,
                                             const FPDualReal & enthalpy) const
{
...
FPDualReal
Water97NoError::temperature_from_ph2a(const FPDualReal & pressure,
                                              const FPDualReal & enthalpy) const
{
  const FPDualReal pi = pressure / 1.0e6;
...
FPDualReal
Water97NoError::temperature_from_ph2b(const FPDualReal & pressure,
                                              const FPDualReal & enthalpy) const
{
  const FPDualReal pi = pressure / 1.0e6;
  const FPDualReal eta = enthalpy / 2000.0e3;
...
FPDualReal
Water97NoError::temperature_from_ph2c(const FPDualReal & pressure,
                                              const FPDualReal & enthalpy) const
{
  const FPDualReal pi = pressure / 1.0e6;
  const FPDualReal eta = enthalpy / 2000.0e3;
...
Real
Water97NoError::b3ab(Real pressure) const
{
  // Check whether the input pressure is within the region of validity of this equation.
  if (pressure < b23p(623.15) || pressure > 100.0e6)
      if (_error)
        mooseException(name(), ": b3ab(): Pressure ", pressure, "is outside range of 16.529 MPa <= p <= 100 MPa");
      else
      {
        // mooseWarning(name(), ": b3ab(): Pressure ", pressure, "is outside range of 16.529 MPa <= p <= 100 MPa",
        //               "Continuing anyway but results may not be valid in this range.");
      }
...
FPDualReal
Water97NoError::temperature_from_ph3a(const FPDualReal & pressure,
                                              const FPDualReal & enthalpy) const
{
...
FPDualReal
Water97NoError::temperature_from_ph3b(const FPDualReal & pressure,
                                              const FPDualReal & enthalpy) const
{
  const FPDualReal pi = pressure / 100.0e6;
...
Real
Water97NoError::henryConstant(Real temperature, const std::vector<Real> & coeffs) const
{
...
void
Water97NoError::henryConstant(Real temperature,
...
DualReal
Water97NoError::henryConstant(const DualReal & temperature,
