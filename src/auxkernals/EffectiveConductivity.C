...
EffectiveConductivity::computeValue()
{
  ...
  for (unsigned int neighbor_idx = 0; neighbor_idx < _neighbor_elem->n_nodes(); ++neighbor_idx)
  {
    ...
    difference = neighbor_enthalpy - _enthalpy[qp]; // Calculate difference

    // Store difference in the property vector
    _enthalpy_difference[qp] = difference;
  }
  ...
  return _enthalpy_difference[qp] / (_length[qp] * _temperature_grad[qp]);
}
